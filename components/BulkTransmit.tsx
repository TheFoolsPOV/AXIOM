...
const BulkTransmit: React.FC<BulkTransmitProps> = ({ 
  ...
  const transmitPacket = async (payload: any, index: number) => {
    ...
    // For GET requests, generated field patterns are appended as query parameters
    if (targetMethod === 'GET' && payload && typeof payload === 'object' && !Array.isArray(payload)) {
    ...
    try {
      // Omit body for GET and DELETE methods as per standard fetch rules
      const hasBody = targetMethod !== 'GET' && targetMethod !== 'DELETE';
      ...
      // Increased snippet limit to show more context in the logs
      const snippetLimit = 120;
      const snippet = responseData.length > snippetLimit ? responseData.substring(0, snippetLimit) + '...' : responseData;
      
      return { 
        status: res.status, 
        text: `PKG #${index + 1}: ${res.statusText} ${snippet ? `[${snippet}]` : ''}`, 
          ...
          {transmissionStatus === 'idle' ? (
            <div className="flex-1 flex flex-col p-8 gap-6 overflow-hidden min-h-0">
              ...
              <div className="flex-1 overflow-hidden min-h-0 relative">
                {viewMode === 'generator' && (
                  <div className="h-full animate-reveal overflow-y-auto custom-scrollbar pr-2">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-full">
                      <div className="lg:col-span-4 flex flex-col gap-6 shrink-0">
                        <div className="glass-card p-6 border border-white/5 bg-[#161b22] rounded-[2rem]">
                        ...
                        <div className="glass-card p-8 bg-black/60 border border-white/10 flex flex-col justify-center items-center text-center rounded-[2.5rem] flex-1 shadow-2xl relative overflow-hidden min-h-[200px]">
                      ...
                      <div className="lg:col-span-8 flex flex-col min-h-0">
                        <div className="glass-card flex-1 p-8 border border-white/5 bg-[#161b22] flex flex-col rounded-[2.5rem] min-h-[300px]">
                          ...
                          <div className="flex-1 overflow-y-auto pr-3 custom-scrollbar space-y-4 pb-4">
                            {fieldPatterns.map((fp, i) => (
                              <div key={fp.id} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center bg-black/40 p-4 rounded-[1.5rem] border border-white/[0.05] hover:border-white/10 transition-all group">
                    ...
                    {parsedArray.map((item: any, idx: number) => (
                      <div key={idx} className="bg-[#161b22] border border-white/[0.03] p-6 rounded-[2rem] flex items-center justify-between group hover:border-white/10 transition-all animate-reveal">
                        <div className="flex items-center gap-8 overflow-hidden">
                          <span className="text-[12px] font-black text-slate-600 tracking-tighter w-24 shrink-0 uppercase">Unit-{(idx + 1).toString().padStart(4, '0')}</span>
                          <div className="w-px h-10 bg-white/5 shrink-0"></div>
                          <code className="text-[13px] text-emerald-400/90 font-mono truncate">{JSON.stringify(item)}</code>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Action Bar */}
              <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-6 mt-2 shrink-0">
                <div className="flex-1 p-5 bg-[#161b22] border border-white/5 rounded-[2.5rem] flex items-center gap-6 shadow-inner overflow-hidden">
                ...
                <button
                  onClick={runBulk}
                  className="px-16 py-6 lg:py-8 rounded-[2.5rem] font-black text-sm uppercase tracking-[0.6em] text-white transition-all active:scale-95 shadow-2xl flex items-center justify-center lg:justify-start gap-6 group hover:brightness-110"
            ...
            <div className="flex-1 flex flex-col p-8 lg:p-16 overflow-hidden bg-[#0d1117] animate-fade">
              <div className="mb-8 lg:mb-16 shrink-0">
                <div className="flex justify-between text-[12px] lg:text-[14px] font-black uppercase text-slate-400 mb-4 lg:mb-8 tracking-[0.6em] min-h-[40px] items-center">
                  ...
                  <span className="text-2xl lg:text-4xl font-black font-mono" style={{ color: transmissionStatus === 'finished' ? '#10b981' : currentAccent }}>
                    {progress}%
                  </span>
                </div>
              ...
              <div className="flex-1 bg-[#0a0d14] rounded-[2rem] lg:rounded-[4rem] p-6 lg:p-12 overflow-y-auto custom-scrollbar space-y-4 border border-white/5 shadow-2xl min-h-0">
                {logs.map((log, i) => (
                  <div key={i} className="flex flex-col lg:flex-row lg:justify-between lg:items-center p-6 bg-white/[0.01] border border-white/[0.04] rounded-[2rem] animate-reveal group hover:bg-white/[0.03] transition-all">
                    <div className="flex items-start lg:items-center gap-6 overflow-hidden mb-4 lg:mb-0">
                       <span className={`text-[12px] font-black px-5 py-2.5 rounded-2xl border shrink-0 ${log.status >= 200 && log.status < 300 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                         {log.status === 0 ? 'FAULT' : log.status}
                       </span>
                       <span className="text-[13px] lg:text-[14px] font-mono text-slate-400 font-bold tracking-tight break-all">
                         {log.text}
                       </span>
                    </div>
                    <span className="text-[12px] font-mono text-slate-600 font-black shrink-0 lg:ml-4 self-end lg:self-center bg-black/40 px-3 py-1 rounded-lg">
                      {log.time}ms
                    </span>
                  </div>
                ))}
              ...
              {transmissionStatus === 'finished' && (
                <div className="mt-8 lg:mt-12 flex gap-4 lg:gap-6 animate-reveal shrink-0">
                  <button onClick={() => setTransmissionStatus('idle')} className="flex-1 py-6 lg:py-8 bg-white/5 hover:bg-white/10 rounded-[2rem] lg:rounded-[3rem] text-[10px] lg:text-sm font-black uppercase tracking-[0.8em] text-slate-200 transition-all border border-white/10 shadow-xl active:scale-95">
                    PREPARE NEW SHIPMENT
                  </button>
                  <button onClick={onClose} className="flex-1 py-6 lg:py-8 rounded-[2rem] lg:rounded-[3rem] text-[10px] lg:text-sm font-black uppercase tracking-[0.8em] text-white transition-all shadow-2xl active:scale-95" style={{ backgroundColor: currentAccent }}>
                    CLOSE HUB
                  </button>
                </div>
              )}
