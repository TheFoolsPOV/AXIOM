      ...
      <div className="glass-panel p-8 border-t-4 bg-black/20 relative overflow-hidden" style={{ borderTopColor: accentColor }}>
        ...
        <div className="flex items-center gap-6 mb-12 pb-10 border-b border-white/5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.4em]">Optimizing Connectivity for Local & Remote APIs</p>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <section>
              ...
              <p className="text-[13px] text-slate-400 leading-relaxed mb-6 font-medium">
                Browsers block scripts from making requests to different origins (domains) unless the server explicitly permits it through headers.
              </p>
              
              <div className="glass-card bg-black/60 p-6 border-white/5 font-mono text-[11px] relative overflow-hidden group">
                <div className="space-y-1 relative z-10">
                  <span className="text-slate-600 select-none italic font-sans">// Example Headers required from server</span><br/>
                  <div className="mt-2 space-y-0.5">
                    ...
                    <span className="text-blue-400">Access-Control-Allow-Headers:</span> <span className="text-emerald-400">Content-Type, Authorization</span>
                  </div>
                </div>
              </div>
            </section>
          ...
          <div className="space-y-10">
            <section className="bg-amber-500/[0.03] border border-amber-500/10 rounded-2xl p-6 relative overflow-hidden">
              ...
              <p className="text-[12px] text-slate-400 leading-relaxed mb-6 font-medium">
                This workbench runs on HTTPS. To call local <code className="text-amber-400/80 px-1 bg-amber-500/10 rounded">http://</code> services, you must manually bypass the browser security guard:
              </p>

              <div className="space-y-3">
                {[
                  { n: "01", text: "Click the 🔒 icon in your browser's address bar" },
                  { n: "02", text: "Open 'Site Settings'" },
                  { n: "03", text: "Locate 'Insecure Content' and set to 'Allow'" }
                ].map(step => (
        ...
        <div className="p-6 glass-card bg-black/40 border-white/5 flex items-center justify-between shadow-xl">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h4 className="text-[8px] text-slate-600 font-black uppercase tracking-[0.3em] mb-1">Transmission State</h4>
              <p className="text-xl font-black text-white tracking-tighter uppercase">Universal Protocol</p>
            </div>
          </div>
          <div className="flex flex-col items-end">
            <div className="flex items-center gap-2 mb-1.5">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_12px_#10b981]"></div>
              <span className="text-[11px] font-black text-emerald-500 uppercase tracking-widest">Axiom Link Active</span>
            </div>
            <span className="text-[8px] text-slate-600 font-black uppercase tracking-widest">Ready for Telemetry...</span>
          </div>
        </div>
      </div>
    </div>
